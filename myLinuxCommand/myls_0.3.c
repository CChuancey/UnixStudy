...
void showFileStat(struct stat* fileStat) {
    ...
    if((S_ISUID&fileStat->st_mode)==S_ISUID){ // set-user-ID bit
        putchar('s');
    }else if((S_IXUSR & fileStat->st_mode)==S_IXUSR) {
        putchar('x');
    }else putchar('-');
    ...
    if((S_ISGID&fileStat->st_mode)==S_ISGID) { //set-group-ID bit
        putchar('s');
    }else if((S_IXGRP & fileStat->st_mode)==S_IXGRP) {
        putchar('x');
    }else putchar('-');
    ...
    if((S_ISVTX&fileStat->st_mode)==S_ISVTX) {
        putchar('t');
    }else if((S_IXOTH & fileStat->st_mode)==S_IXOTH) {
        putchar('x');
    }else putchar('-');
